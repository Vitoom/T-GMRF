...
```bash
pip install sklearn, snap, tslearn, tqdm, hdbscan, pyreadr, pathlib, 'ray[tune]'
```
...
+ [**FCFW**](https://github.com/Vitoom/T-GMRF/tree/main/Baselines/FCFW): KMeans Clustering on a hybrid fuzzy membership matrix generated by performing DTW on all variables and reconsidering each dimension’s shape based distance (SBD). 
