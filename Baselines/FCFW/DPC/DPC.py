...
class DPC(object):
    ...
    def get_centers(self, data, data_name="dataset", n=6, dc_method=0, dc_percent=1, rho_method=1, delta_method=1, use_halo=False, plot=None):
        # 读取点，计算距离
        points, d_matrix, d_list, min_dis, max_dis, max_id = self.load_points_cacl_distance(data)
        # 计算截断聚类 dc
    ...
    def load_points_cacl_distance(self, data):
        #print(path)
        points = data # pd.read_csv(path, sep='\t', usecols=[0, 1])
        max_id = len(points)
        d = pd.DataFrame(np.zeros((max_id, max_id)))  # 距离矩阵
        
        _points = points.reshape(points.shape[0], -1)
        dis = sch.distance.pdist(_points, 'cityblock')  # 欧式距离 euclidean'
        
        # dis = np.zeros((max_id, max_id))
        
        # for i in range(max_id):
        #     for j in range(i+1, max_id):
        #         # for k in range(45):
        #         #    tmp, _ = fastdtw(X[i,k], X[j,k], dist=_metric)
        #         dis[i, j] = sum(abs(data[i].reshape(-1) - data[j].reshape(-1)))
        #         dis[j, i] = dis[i, j]
